...
const updateTimer = (currentPlayer, minutes, seconds) => {
    if(currentPlayer === 'light'){
        playerLightTimer.innerText = `${minutes >= 10 ? minutes : "0" + minutes}:${seconds >= 10 ? seconds : "0" + seconds}`
    } else {
        playerBlackTimer.innerText = `${minutes >= 10 ? minutes : "0" + minutes}:${seconds >= 10 ? seconds : "0" + seconds}`
    }
}
...
const performElPassant = (currentPlayer, prevPawnPosition, newPawnPosition) => {
    ...
    capturePiece(capturePiece.children[0])
...
const endGame = (winner=null) => {
    ...
    if(winner){
        // finish game over functionality 
    } else {
        winnerUsername.innerHTML = "nobody"
    }
